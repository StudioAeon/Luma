...
#include "Luma/Renderer/Backend/Vulkan/VulkanFramebuffer.hpp"
#include "Luma/Renderer/Backend/Vulkan/VulkanShader.hpp"
...
namespace Luma {
	...
	void SceneRenderer::Init()
	{
		...
		s_Data->BloomBlurShader = Shader::Create("Resources/Shaders/BloomBlur.glsl");
		s_Data->BloomBlendShader = Shader::Create("Resources/Shaders/BloomBlend.glsl");
#endif
		...
		// Geometry
		{
			...
			pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("PBR_StaticMesh");
	...
	{
		LM_CORE_ASSERT(!s_Data->ActiveScene, "");
	...
	void SceneRenderer::OnImGuiRender()
	{
		...
		if (UI::BeginTreeNode("Shadows"))
		{
			...
			if (UI::BeginTreeNode("Shadow Map", false))
			{
				...
				float size = std::min(availRegion.x, availRegion.y); // (float)fb->GetWidth() * 0.5f, (float)fb->GetHeight() * 0.5f
				UI::BeginPropertyGrid();
