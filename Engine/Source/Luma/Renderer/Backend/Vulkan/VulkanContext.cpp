...
namespace Luma {
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL VulkanDebugReportCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
	{
		...
		std::string formatted = std::format("VulkanDebugCallback:\n  Object Type: {}\n  Message: {}", static_cast<int>(objectType), pMessage);

		LM_CORE_WARN("{}", formatted);
		return VK_FALSE;
	}
	...
	{
		LM_CORE_INFO("VulkanContext::Create");

		LM_CORE_ASSERT(SDL_Vulkan_LoadLibrary(nullptr), "Failed to load Vulkan library!");
		...
		appInfo.pApplicationName = "Luma";
		appInfo.pEngineName = "Luma";
...
#define VK_KHR_WIN32_SURFACE_EXTENSION_NAME "VK_KHR_win32_surface"
		...
		if (s_Validation)
		{
			...
			LM_CORE_TRACE("Vulkan Instance Layers:");
			for (const VkLayerProperties& layer : instanceLayerProperties)
			{
				LM_CORE_TRACE("  {0}", layer.layerName);
			...
			{
				LM_CORE_ERROR("Validation layer VK_LAYER_LUNARG_standard_validation not present, validation is disabled");
			}
		}
		...
		if (s_Validation)
		{
			...
			LM_CORE_ASSERT(vkCreateDebugReportCallbackEXT != NULL, "");
