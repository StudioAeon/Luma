...
namespace Luma {
	...
	void VulkanSwapChain::InitSurface(SDL_Window* windowHandle)
	{
		...
		{
			LM_CORE_ERROR("Failed to create Vulkan surface: {}", SDL_GetError());
			LM_CORE_ASSERT(false, "Failed to create Vulkan surface!");
		}

		LM_CORE_ASSERT(m_Surface != VK_NULL_HANDLE, "Surface creation returned null handle!");

		// Get available queue family properties
		uint32_t queueCount;
		vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, &queueCount, NULL);
		LM_CORE_ASSERT(queueCount >= 1);
		...
		LM_CORE_ASSERT(graphicsQueueNodeIndex != UINT32_MAX);
		LM_CORE_ASSERT(presentQueueNodeIndex != UINT32_MAX);
	...
	void VulkanSwapChain::Create(uint32_t* width, uint32_t* height, bool vsync)
	{
		...
		LM_CORE_ASSERT(presentModeCount > 0, "");
	...
	{
		LM_CORE_WARN("VulkanContext::OnResize");
	...
	{
		LM_SCOPE_PERF("VulkanSwapChain::BeginFrame");
	...
	{
		LM_SCOPE_PERF("VulkanSwapChain::Present");
		...
		VkResult result;
		{
			LM_SCOPE_PERF("VulkanSwapChain::Present - QueuePresent");
