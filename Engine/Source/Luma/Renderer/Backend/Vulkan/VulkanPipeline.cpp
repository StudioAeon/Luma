...
namespace Luma {
	...
	void VulkanPipeline::Invalidate()
	{
		...
		Renderer::Submit([instance]() mutable
		{
			...
			LM_CORE_ASSERT(instance->m_Specification.Shader);
			...
			if (!shaderDescriptorSets.empty())
			{
				...
				LM_CORE_WARN("VulkanPipeline - Updating {0} descriptor sets", writeDescriptors.size());
	...
	void VulkanPipeline::RT_SetUniformBuffer(Ref<UniformBuffer> uniformBuffer, uint32_t binding, uint32_t set)
	{
		...
		LM_CORE_ASSERT(m_DescriptorSets.DescriptorSets.size() > set);
		...
		LM_CORE_WARN("VulkanPipeline - Updating descriptor set (VulkanPipeline::SetUniformBuffer)");
		VkDevice device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();
