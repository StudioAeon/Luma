...
namespace Luma {
	...
	namespace Utils {
		...
		{
			return std::filesystem::path("Resources/cache/shader/opengl");
		}
		...
		static void CreateCacheDirectoryIfNeeded()
		{
			...
			if (!FileSystem::Exists(cacheDirectory))
			{
				...
				LM_CORE_INFO("Created shader cache directory: {0}", cacheDirectory.string());
			}
		}
		...
		static ShaderUniformType SPIRTypeToShaderUniformType(spirv_cross::SPIRType type)
		{
			...
			LM_CORE_ASSERT(false, "Unknown type!");
	...
	void VulkanShader::Reflect(VkShaderStageFlagBits shaderStage, const std::vector<uint32_t>& shaderData)
	{
		...
		LM_CORE_TRACE("===========================");
		LM_CORE_TRACE(" Vulkan Shader Reflection");
		LM_CORE_TRACE(" {0}", m_AssetPath);
		LM_CORE_TRACE("===========================");

		spirv_cross::Compiler compiler(shaderData);
		auto resources = compiler.get_shader_resources();

		LM_CORE_TRACE("Uniform Buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			LM_CORE_TRACE("  {0} ({1}, {2})", name, descriptorSet, binding);
			LM_CORE_TRACE("  Member Count: {0}", memberCount);
			LM_CORE_TRACE("  Size: {0}", size);
			LM_CORE_TRACE("-------------------");
		}

		LM_CORE_TRACE("Push Constant Buffers:");
		for (const auto& resource : resources.push_constant_buffers)
		{
			...
			LM_CORE_TRACE("  Name: {0}", bufferName);
			LM_CORE_TRACE("  Member Count: {0}", memberCount);
			LM_CORE_TRACE("  Size: {0}", bufferSize);
	...
	void VulkanShader::CreateDescriptors()
	{
		...
		for (auto&& [set, shaderDescriptorSet] : m_ShaderDescriptorSets)
		{
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.ImageSamplers)
			{
				...
				LM_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
			...
			for (auto& [bindingAndSet, imageSampler] : shaderDescriptorSet.StorageImages)
			{
				...
				LM_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
				LM_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
			...
			LM_CORE_INFO("Creating descriptor set {0} with {1} ubos, {2} samplers and {3} storage images", set,
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set)
	{
		...
		LM_CORE_ASSERT(m_TypeCounts.find(set) != m_TypeCounts.end());
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set, uint32_t numberOfSets)
	{
		...
		LM_CORE_ASSERT(poolSizes.find(set) != poolSizes.end());
		...
		for (auto [stage, source] : m_ShaderSource)
		{
			...
			if (outputBinary[stage].size() == 0)
			{
				...
				// Compile shader
				{
					...
					{
						LM_CORE_ERROR("{}", module.GetErrorMessage());
						LM_CORE_ASSERT(false);
					}
