...
namespace Luma {
		...
		: m_Path(path), m_Properties(properties)
	{
		...
		LM_CORE_ASSERT(m_ImageData.Data, "Failed to load image!");
		if (!m_ImageData.Data)
			return;
		...
		LM_CORE_ASSERT(m_Format != ImageFormat::None);
		...
		: m_Format(format), m_Properties(properties)
	{
		...
		LM_CORE_ASSERT(format == ImageFormat::RGBA);
	...
	void VulkanTexture2D::Invalidate()
	{
		...
		LM_CORE_ASSERT(m_ImageData.Data);
	...
	VulkanTextureCube::~VulkanTextureCube()
	{
		...
		{
			LM_CORE_TRACE("Destroying VulkanTextureCube");
