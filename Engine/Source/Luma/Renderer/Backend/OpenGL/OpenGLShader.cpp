...
#define PRINT_SHADERS 1
		...
		for (auto [stage, source] : m_ShaderSource)
		{
			...
			if (outputBinary[stage].size() == 0)
			{
				...
				// Compile shader
				{
					...
					{
						LM_CORE_ERROR("Shader compilation {}", module.GetErrorMessage());
						LM_CORE_ASSERT(false);
					}
	...
	void OpenGLShader::CompileOrGetOpenGLBinary(const std::unordered_map<uint32_t, std::vector<uint32_t>>& vulkanBinaries, bool forceCompile)
	{
		...
		for (auto& [bufferName, buffer] : m_Buffers)
		{
			for (auto& [name, uniform] : buffer.Uniforms)
			{
				...
				if (location == -1)
					LM_CORE_WARN("{0}: could not find uniform location {0}", name);
	...
	void OpenGLShader::Reflect(std::vector<uint32_t>& data)
	{
		...
		LM_CORE_TRACE("OpenGLShader::Reflect - {0}", m_AssetPath);
		LM_CORE_TRACE("   {0} Uniform Buffers", res.uniform_buffers.size());
		LM_CORE_TRACE("   {0} Resources", res.sampled_images.size());
		...
		for (const spirv_cross::Resource& resource : res.uniform_buffers)
		{
			...
			if (s_UniformBuffers.find(bindingPoint) == s_UniformBuffers.end())
			{
				...
				LM_CORE_TRACE("Created Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes", buffer.BindingPoint, buffer.Name, buffer.Size);
			...
			else
			{
				...
				LM_CORE_ASSERT(buffer.Name == resource.name); // Must be the same buffer
				if (bufferSize > buffer.Size) // Resize buffer if needed
				{
					...
					LM_CORE_TRACE("Resized Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes", buffer.BindingPoint, buffer.Name, buffer.Size);
				}
			}
		}
	...
	void OpenGLShader::SetUniformBuffer(const std::string& name, const void* data, uint32_t size)
	{
		...
		LM_CORE_ASSERT(uniformBuffer);
		LM_CORE_ASSERT(uniformBuffer->Size >= size);
	...
	void OpenGLShader::SetUniform(const std::string& fullname, float value)
	{
		Renderer::Submit([=]()
		{
			LM_CORE_ASSERT(m_UniformLocations.find(fullname) != m_UniformLocations.end());
	...
	void OpenGLShader::UploadUniformFloat(const std::string& name, float value)
	{
		...
		if (location != -1)
			...
			LM_LOG_UNIFORM("Uniform '{0}' not found!", name);
	}
