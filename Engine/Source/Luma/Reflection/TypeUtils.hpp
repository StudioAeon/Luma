...
namespace Luma::Type {
	...
	namespace impl
	{
		...
		struct is_callable_impl : public std::is_function<std::remove_reference_t<std::remove_pointer_t<T>>>::type {};
	...
	template<typename T>
	using is_callable_t = typename impl::is_callable_impl<std::is_class<std::remove_reference_t<T>>::value, std::remove_reference_t<T> >::type;

	template<typename T>
	static constexpr bool is_callable_v = is_callable_t<T>::value;
