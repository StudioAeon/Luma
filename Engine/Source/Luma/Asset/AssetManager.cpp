...
namespace Luma {
	...
	void AssetManager::Rename(AssetHandle assetHandle, const std::string& newName)
	{
		...
		std::filesystem::path newPath = oldPath.parent_path() / (newName + "." + asset->Extension);
		...
		{
			LM_CORE_ERROR("Failed to rename asset: {0}", e.what());
		}
	}
	...
	void AssetManager::LoadAssetRegistry()
	{
		...
		if (!handles)
		{
			LM_CORE_ERROR("Failed to read Asset Registry file.");
			return;
		}
		...
		for (auto entry : handles)
		{
			...
			{
				LM_CORE_WARN("Tried to load metadata for non-existing asset: {0}", metadata.FilePath);
				continue;
			}

			if (metadata.Handle == 0)
			{
				LM_CORE_WARN("AssetHandle for {0} is 0, this shouldn't happen.", metadata.FilePath);
				continue;
			}
	...
	Ref<Asset> AssetManager::CreateAsset(const std::string& filepath, AssetType type, AssetHandle parentHandle)
	{
		...
		if (s_AssetRegistry.find(asset->FilePath) != s_AssetRegistry.end())
		{
			...
			if (asset->Type != type)
			{
				LM_CORE_WARN("AssetType for '{0}' was different than the metadata. Did the file type change?", asset->FilePath);
		...
		asset->FileName = Utils::RemoveExtension(std::string(Utils::GetFilename(asset->FilePath)));
