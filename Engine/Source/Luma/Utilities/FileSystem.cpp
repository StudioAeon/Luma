...
namespace Luma {
	...
	uint64_t FileSystem::GetLastWriteTime(const std::filesystem::path& filepath)
	{
		...
		//LM_CORE_ASSERT(FileSystem::Exists(filepath));
		...
		LM_CORE_ERROR("FileSystem::GetLastWriteTime - could not open file: {}", filepath.string());
		return 0;
	}
	...
	std::filesystem::path FileSystem::OpenFileDialog(const std::initializer_list<FileDialogFilterItem> inFilters)
	{
		...
		switch (result)
		{
			...
			case NFD_ERROR:
			{
				LM_CORE_VERIFY(false, "NFD-Extended threw an error: {}", NFD::GetError());
				return "";
			}
		}
	}
