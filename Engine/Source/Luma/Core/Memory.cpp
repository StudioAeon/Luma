...
namespace Luma {
	...
	static Luma::AllocationStats s_GlobalStats;
...
#ifndef LM_DIST
			if (!found)
				LM_CORE_WARN_TAG("Memory", "Memory block {0} not present in alloc map", memory);
#endif
		}
...
#if defined(LM_TRACK_MEMORY) && defined(LM_PLATFORM_WINDOWS)

_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size) _VCRT_ALLOCATOR
void* __CRTDECL operator new(size_t size)
{
	return Luma::Allocator::Allocate(size);
}
...
{
	return Luma::Allocator::Allocate(size, desc);
}
...
{
	return Luma::Allocator::Allocate(size, file, line);
}
...
{
	return Luma::Allocator::Free(memory);
}
