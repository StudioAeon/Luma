...
namespace Luma {
	...
	{
	public:
		LM_FORCE_INLINE Timer() { Reset(); }
		LM_FORCE_INLINE void Reset() { m_Start = std::chrono::high_resolution_clock::now(); }
		LM_FORCE_INLINE float Elapsed() { return std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - m_Start).count() * 0.001f * 0.001f; }
		LM_FORCE_INLINE float ElapsedMillis() { return std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - m_Start).count() * 0.001f; }
	private:
	...
	public:
		...
		~ScopedTimer()
		{
			...
			LM_CORE_TRACE_TAG("Timer", "{0} - {1}ms", m_Name, time);
		}
	private:
		std::string m_Name;
		Timer m_Timer;
