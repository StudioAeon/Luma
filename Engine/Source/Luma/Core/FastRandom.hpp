...
namespace Luma
{
	...
	struct FastRandom
	{
		...
		inline unsigned int		GetUInt32() noexcept { return static_cast<unsigned int>(GetInt32()); }
		inline short			GetInt16() noexcept { return static_cast<short>(GetInt32() & 0xFFFF); }
		inline unsigned short	GetUInt16() noexcept { return static_cast<unsigned short>(GetInt16() & 0xFFFF); }
		inline double			GetFloat64() noexcept { return GetInt32() / static_cast<double>(0x7FFFFFFF); }
		inline float			GetFloat32() noexcept { return static_cast<float>(GetFloat64()); }
	...
	namespace Utils {
		...
		inline int GetSeedFromCurrentTime() noexcept
		{
			...
			// NOTE: Time is used for seed, so 32-bits is fine
